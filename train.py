...
def test_full(model, test_loader, test_lb, test_ub, batch_size, **kwargs):
    model.eval()

    ret = torch.empty(test_lb.shape[0], device=config.device)
    for i, (g, h, lb_gt, ub_gt) in enumerate(tqdm(test_loader, 'batches')):
        g = g.to(config.device)
        h = h.to(config.device)
        with torch.no_grad():
            pred, _ = model.forward(g, h)
            ret[i * batch_size:(i + 1) * batch_size] = pred

    print("test metrics", end=' / ')
    print(f'rmse {metrics.rmse(test_lb, test_ub, ret):.3f}', end=' / ')
    print(f'mae {metrics.mae(test_lb, test_ub, ret):.3f}', end=' / ')
    print(f'r2 {metrics.r2(test_lb, test_ub, ret) * 100:.2f}')
...
def train_full(model, loader, val_loader, lr, weight_decay, cycle_patience, step_size_up, step_size_down, dump_path=None, lrs=None, max_grad_norm=config.max_grad_norm):
    ...
    while True:
        ...
        for batch, val_batch in zip(loader, it.cycle(val_loader)):
            ...
            val_g, val_h, val_lb, val_ub = (x.to(config.device) for x in val_batch)
            with torch.no_grad():
                val_pred, val_loss = model(val_g, val_h, val_lb, val_ub)
            val_losses.append(val_loss.item())
