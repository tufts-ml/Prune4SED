...
from torch_geometric.nn.pool.topk_pool import uniform
from torch_geometric.nn.pool.topk_pool import maybe_num_nodes

from torch_geometric.utils import k_hop_subgraph as tg_k_hop_subgraph
...
class TopKPooling(torch.nn.Module):
    ...
    def forward(self, x, edge_index, left_hop, edge_attr=None, batch=None, attn=None):
        """"""
        ...
        perm, edge_index, _, _ = tg_k_hop_subgraph(perm, left_hop, edge_index, relabel_nodes=True, num_nodes=score.size(0))

        x = x[perm] * score[perm].view(-1, 1)
        batch = batch[perm]

        return x, edge_index, batch, perm, score[perm], score.detach()
